...
def test_repo_has_exactly_eight_valid_skills(cfg, skills_dir):
    skills, errors = validate_skills_dir(skills_dir, cfg)
    assert errors == []
    assert sorted(skills) == [1, 2, 3, 4, 5, 6, 7, 8]
...
def test_extra_skill_directory_is_rejected(cfg, skills_dir, tmp_path):
    ...
    (copy / "09-extra").mkdir()
    (copy / "09-extra" / "SKILL.md").write_text("---\nname: x\nphase: 4\nversion: 1.0.0\ndescription: d\n---\n" + "x" * 500, encoding="utf-8")
    _, errors = validate_skills_dir(copy, cfg)
    assert any("unexpected skill directory `09-extra`" in e for e in errors)
...
def test_intent_explicit_and_trailer(cfg):
    assert detect_intent(cfg, explicit="deploy").phases == [3, 4, 5, 6, 7, 8]
...
def test_intent_branch_patterns(cfg):
    ...
    assert detect_intent(cfg, branch="hotfix-login", paths=["src/a.py"]).phases == [4, 5, 6, 7, 8]
...
def test_intent_default(cfg):
    ...
    assert d.intent == "commit" and d.phases == [3, 4, 5, 8] and d.source == "default"
