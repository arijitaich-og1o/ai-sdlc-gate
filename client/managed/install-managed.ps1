<#
.SYNOPSIS
  SDLC Gate — MANAGED client install for Windows (run as Administrator, typically via Intune / SCCM / GPO).

.DESCRIPTION
  Installs the gate where standard users cannot change it:
    C:\ProgramData\sdlc-gate\repo     engine, skills, policy (Administrators: Full; Users: Read)
    C:\ProgramData\sdlc-gate\venv     isolated Python with the sdlc-gate CLI
    C:\ProgramData\sdlc-gate\hooks    git hooks (bash, executed by Git for Windows' sh)
    C:\ProgramData\sdlc-gate\bin      git.cmd shim (strips --no-verify / -n on commit, forces managed hooks path)
    system gitconfig                  core.hooksPath -> managed hooks; sdlc-gate.managed=true (fail closed + identity required)
...
param(
  ...
  [string]$Prefix = "C:\ProgramData\sdlc-gate"
)
...
Copy-Item -Force (Join-Path $venv "Scripts\sdlc-gate.exe") (Join-Path $Prefix "bin\sdlc-gate.exe")
...
$shim = @"
@echo off
rem SDLC Gate managed git shim: forces the managed hooks path and removes hook-bypass flags.
...
if "!SKIPC!"=="1" (
  set "SKIPC=0"
  echo %~1 | findstr /I /B "core.hooksPath core.hookspath sdlc-gate." >nul && goto next
  set "ARGS=!ARGS! -c %1"
  goto next
)
...
git config --system sdlc-gate.prefix ($Prefix -replace "\\", "/")
