...
```bash
sdlc-gate configure          # one-time: fetches the review configuration from the central repository's secrets
...
| **Spend limit and rotation** | The shared key should be a gateway virtual key with a spend limit. Rotate it by updating the secret; clients pick the new value up with `sdlc-gate configure`. Gateway logs attribute usage to the key, not to a person; per-person attribution comes from the gate's own metrics. |
