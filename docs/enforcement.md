...
|---|---|---|
| **1. GitHub ruleset** (`templates/org-ruleset.json`) | Requires the `SDLC Gate` check on every protected branch, with no bypass actors. Enterprise plans can additionally *require the workflow itself*, so a repository that never added the caller still gets gated. | **No.** Not by editing files, not by deleting the caller, not by force-pushing. This is the guarantee. |
