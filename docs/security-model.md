...
| Developer bypasses the gate (unmanaged machine, local admin) | Commits lack the `AI-SDLC-Gate-Client` attestation and runs stop appearing in metrics; the scoreboard shows the attested share per developer. |
...
| Token theft from workflows | Least-privilege `permissions` per job; `persist-credentials: false` on checkouts; `pull_request_target` banned; secrets never echoed; report comments use the ephemeral job token. `AI_SDLC_GATE_TOKEN` is scoped to this repository only. |
